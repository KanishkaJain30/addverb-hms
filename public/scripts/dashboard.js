    ...
    $.getJSON(roomsAPI).done(function (rooms1) {
      ...
      $('#patients-waiting').dataTable({
        ...
        columns: [
          {
            title:
              '<span class="fa fa-hospital-o fa-fw" style="color: black;"></span>   ' +
              '  no.',
            width: '30%',
          },
          {
      ...
      var patientsWithRoomsDashboard =
        patientsInHospitalTableConstructor.length || 0;
      $('#patients-with-rooms-live').html(patientsWithRoomsDashboard);

      var patientsWaitingDashboard =
        patientsWaitingTableConstructor.length || 0;
      ...
      if (clicks_when_called + 1 === clicks) {
        if (confirm('Do you want to make the change?')) {
          window.location.href =
            URL +
            '/app/swappatients/' +
            hospitalNumberToBeWaiting +
            '/' +
            hospitalNumberToBeAdmitted;
        } else {
