...
$(document).ready(function () {
  ...
  $.getJSON(patientAPI).done(function (patient) {
    ...
    if (patient['room'] === 'noroom') {
      ...
      $('#patient-room-disabled').attr(
        'placeholder',
        'Room: ' + patient['room']
      );

      var patientDeleteRoomLink =
        '/app/updateroom/' + hospitalNumber[3] + '/noroom';
      $('#patient-room-disabled').after(
        '<a id="delete-room-button" class="btn btn-primary btn-lg btn-block" href="' +
          patientDeleteRoomLink +
          '">Move to waiting list</a>'
      );
    }
        ...
        if (patient['diseases'].length !== 0) {
          for (var i = 0; i < patient['diseases'].length; i++) {
            if (disease === patient['diseases'][i]) {
              input =
                '<input type="checkbox" name="PD[]" value="' +
                disease +
                '" checked>';
              break;
            } else {
              input =
                '<input type="checkbox" name="PD[]" value="' + disease + '">';
            }
          }
        } else {
